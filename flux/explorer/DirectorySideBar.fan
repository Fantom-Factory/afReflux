...
internal class DirectorySideBar : SideBar {
...
//		if (state.selected != null) select(state.selected)
		...
		addTree(FileResource(`file:C:/Projects/`.toFile))
	}
...
//		state.save
	...
	** tree root.
	Void addTree(Resource? r) {
		...
		name	:= r == null ? "My Computer" : r.name
		items := combo.items.size == 0 ? combo.items.dup : combo.items[0..<-1]
		combo.items = items.add(name).add("Edit this list...")
...
//			frame.load(event.data->uri, LoadMode(event))
		fireEvent("dirOpened", event.data->uri)
	}
...
//	}
	...
	internal Void editList() {
		...
		if (dlg.open == Dialog.ok) {
			...
			trees = newTrees
			ignore = true
			combo.items = items.add("Edit this list...")
			combo.selectedIndex = index
			ignore = false
			select(index)
		}
